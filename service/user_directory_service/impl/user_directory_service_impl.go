...
func (userService *UserDirectoryServiceImpl) LookUpCurrentWriteShard(userID string) (*model.DatabaseShard, errorlib.AppError) {
	...
	client := database_clustermgt_client.NewDatabaseClusterMgtClient(databaseClstrMgtMsCfg.Host, databaseClstrMgtMsCfg.Port)
	shardPtr, appErr := client.FindCurrentWriteShard(database.UsersTable, userID)
	if appErr != nil {
		return nil, appErr
	}
	return shardPtr, nil
